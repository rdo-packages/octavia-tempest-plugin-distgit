...
echo "XXX debug debuginfo"
cat /usr/lib/rpm/find-debuginfo.sh
...
CGO_ENABLED=0 GOOS=linux go build -o %{plugin}-tests-httpd -ldflags "-s -w -compressdwarf=false ${LDFLAGS:-} -B 0x$(head -c20 /dev/urandom|od -An -tx1|tr -d ' \n') -extldflags '-static %__global_ldflags'" -a -v -x test_server.go
popd
